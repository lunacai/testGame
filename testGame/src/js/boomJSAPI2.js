...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
/* Merging js from "file.txt" begins */
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */


/* Last merge : 2018年 6月19日 星期二 16时52分04秒 CST  */
...
KBEngine.Event = function()
{
    ...
    this.getInitData = function(loginType) {
        KBEngine.app.clientVersion = "1.0.1";
        KBEngine.app.clientScriptVersion = "0.1.0";
        KBEngine.app.encryptedKey = "";

        var bundle = new KBEngine.Bundle();
        
        if(loginType == "loginapp")
        ...
        var data = bundle.send(KBEngine.app); 
        return data;
    }

    this.getLoginClientData = function(username, password, datas, func) {
    ...
    this.getLoginBaseData = function(username, password) {
        KBEngine.app.username = username;
        KBEngine.app.password = password;
        ...
        bundle.writeString(KBEngine.app.username);
        bundle.writeString(KBEngine.app.password);
        var back = bundle.send(KBEngine.app);
        return back;
    }
    ...
    this.getBaseImportData = function() {
        var bundle = new KBEngine.Bundle();
...
KBEngine.messages["onLoginSuccessfully"] = new KBEngine.Message(502, "onLoginSuccessfully", 0, 0, new Array(3), null);
KBEngine.clientmessages = {
    502: KBEngine.messages["onLoginSuccessfully"]
};

KBEngine.messages["Loginapp_importClientMessages"] = new KBEngine.Message(5, "importClientMessages", 0, 0, new Array(), null);
KBEngine.messages["Baseapp_importClientMessages"] = new KBEngine.Message(7, "importClientMessages", 0, 0, new Array(), null);
...
KBEngine.messages["Loginapp_login"] = new KBEngine.Message(3, "Loginapp_login", -1, -1, new Array(), null);
KBEngine.messages["Baseapp_loginBaseapp"] = new KBEngine.Message(202, "Baseapp_loginBaseapp", -1, -1, new Array(), null);
KBEngine.messages["Loginapp_hello"] = new KBEngine.Message(4, "Loginapp_hello", -1, -1, new Array(), null);
KBEngine.messages["Baseapp_hello"] = new KBEngine.Message(200, "Baseapp_hello", -1, -1, new Array(), null);
...
KBEngine.KBEngineApp = function(kbengineArgs)
{
    ...
    this.installEvents = function()
    {
        ...
        KBEngine.Event.register("createAccount", KBEngine.app, this.createAccount);
        KBEngine.Event.register("login", KBEngine.app, this.login);
        KBEngine.Event.register("reloginBaseapp", KBEngine.app, this.reloginBaseapp);
        KBEngine.Event.register("bindAccountEmail", KBEngine.app, this.bindAccountEmail);
        KBEngine.Event.register("newPassword", KBEngine.app, this.newPassword);
    }
    ...
    */
    ...
    this.onImportClientMessagesCompleted = function()
    {
        ...
        KBEngine.Event.fire("onImportClientMessagesCompleted");
    ...
    this.Client_onImportClientEntityDef = function(stream)
    {
        ...
        while(!stream.readEOF())
        {
            ...
            try
            {
                // var defmethod = eval("KBEngine." + scriptmodule_name);
                var defmethod = KBEngine.Account;
            }
            catch(e)
            {
    ...
    this.createAccount_loginapp = function(noconnect)
    {
        if(noconnect)
        {
            KBEngine.INFO_MSG("KBEngineApp::createAccount_loginapp: start connect to " + KBEngine.app.wstype + "://" + KBEngine.app.ip + ":" + KBEngine.app.port + "!");
            KBEngine.app.connect(KBEngine.app.wstype + "://" + KBEngine.app.ip + ":" + KBEngine.app.port);
    ...
    this.initCreate = function(_wstype, _ip, _port) {
        KBEngine.app.connect(_wstype + "://" + _ip + ":" + _port);
    ...
    this.login_loginapp = function(noconnect)
    {
        if(noconnect)
        {
            KBEngine.INFO_MSG("KBEngineApp::login_loginapp: start connect to " + KBEngine.app.wstype + "://" + KBEngine.app.ip + ":" + KBEngine.app.port + "!");
            KBEngine.app.connect(KBEngine.app.wstype + "://" + KBEngine.app.ip + ":" + KBEngine.app.port);
            KBEngine.app.socket.onopen = KBEngine.app.onOpenLoginapp_login;
        }
        else
    ...
    this.login_baseapp = function(noconnect)
    {
        if(noconnect)
        {
            ...
            KBEngine.INFO_MSG("KBEngineApp::login_baseapp: start connect to " + KBEngine.app.wstype + "://" + KBEngine.app.baseappIp + ":" + KBEngine.app.baseappPort + "!");
            KBEngine.app.connect(KBEngine.app.wstype + "://" + KBEngine.app.baseappIp + ":" + KBEngine.app.baseappPort);
    ...
    this.reloginBaseapp = function()
    {
        ...
        KBEngine.INFO_MSG("KBEngineApp::reloginBaseapp: start connect to " + KBEngine.app.wstype + "://" + KBEngine.app.baseappIp + ":" + KBEngine.app.baseappPort + "!");
        KBEngine.app.connect(KBEngine.app.wstype + "://" + KBEngine.app.baseappIp + ":" + KBEngine.app.baseappPort);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
/* Merging js: jsAPI.js begins */
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

function getInitData(loginType) {
    var args = new KBEngine.KBEngineArgs();
    KBEngine.create(args);
    var data = KBEngine.Event.getInitData(loginType);
    return getOriginBytes(data);
}

function getLoginData(_userName, _userPass, _data) {
    var args = new KBEngine.KBEngineArgs();
    KBEngine.create(args);
    var data = KBEngine.Event.getLoginClientData(_userName,_userPass,_data);
    return getOriginBytes(data);
}

function getLoginBaseData(_userName, _userPass) {
    var args = new KBEngine.KBEngineArgs();
    KBEngine.create(args);
    var data = KBEngine.Event.getLoginBaseData(_userName,_userPass);
    return getOriginBytes(data);
}
...
function getImportData() {
    var args = new KBEngine.KBEngineArgs();
    KBEngine.create(args);
    var data = KBEngine.Event.getImportData();
    return getOriginBytes(data);
}

function getBaseImportData() {
    var args = new KBEngine.KBEngineArgs();
    KBEngine.create(args);
    var data = KBEngine.Event.getBaseImportData();
    return getOriginBytes(data);
}
...
function onMessage(msg) {
    ...
    while(stream.rpos < stream.wpos)
    {
        ...
        else
        {
            ...
            var rpos = stream.rpos + msglen;
            stream.wpos = rpos;
            var result = msgHandler.createFromStream(stream);
            // msgHandler.handleMessage(stream);
            stream.wpos = wpos;
            stream.rpos = rpos;
            return result;
...
function getOriginBytes(buffer) {
